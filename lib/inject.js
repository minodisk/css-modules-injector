...
const injectWithOptions = (options) => {
  return Promise
    ...
    .then((arg) => {
      const inputGlob = arg[0]
      const cssGlob = arg[1]
      if (inputPaths.length === 0) {
        throw new Error('no inputs')
      }
      return injectWithFiles(inputGlob, cssGlob, options.output)
    })
}

const injectWithFiles = (inputGlob, cssGlob, outputDir) => {
  console.log(inputGlob, cssGlob)
  const sources = inputGlob.paths.map((sourcePath) => {
  ...
  const cssObjs = cssGlob.paths.map((cssPath) => {
